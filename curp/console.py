...
manual = {
    ...
    "sum-acf": ("Average auto-correlation function over the given " "trajectories"),
}
...
def arg_curp():
    ...
    parser = argparse.ArgumentParser(description=manual["curp"], prog="curp")

    # Add commands as subparsers.
    sp = parser.add_subparsers(dest="command", help="Curp commands")
    sp_compute = sp.add_parser("compute", help=manual["compute"])
    sp_cal_tc = sp.add_parser("cal-tc", help=manual["cal-tc"])
    sp_graph_een = sp.add_parser("graph-een", help=manual["graph-een"])
    sp_sum_acf = sp.add_parser("sum-acf", help=manual["sum-acf"])

    from curp.script.console import manual as script_manual

    sp_conv_trj = sp.add_parser("conv-trj", help=script_manual["conv-trj"])
    sp_analyze = sp.add_parser("analyze", help=script_manual["analyze"])

    from curp.tool.console import manual as tool_manual

    sp_tool = sp.add_parser("tool", help=tool_manual["tool"])
...
def arg_compute(parser=None):
    ...
    parser.add_argument(
        "--output-conf-default",
        dest="output_conf_def",
        default=False,
        action="store_true",
        help=("Output the config parameters in ini format " "with default values."),
    )

    parser.add_argument(
        "--output-conf-formatted",
        dest="output_conf_fmtd",
        default=False,
        action="store_true",
        help=("Output the config parameters in rest style " "with default values."),
    )

    parser.add_argument(
        "input_", nargs="?", default="run.cfg", help="Specify input filenames."
    )
...
def arg_cal_tc(parser=None):
    ...
    parser.add_argument(
        dest="flux_fn", metavar="FLUX_FILENAME", help=("The filepath of flux data.")
    )
...
def arg_graph_een(parser=None):
    ...
    # Add argument definitions
    parser.add_argument("data_fns", nargs="+", help="Specify filenames. ")
    parser.add_argument(
        "-t",
        "--targets",
        dest="targets",
        ...
        default=["1-"],
        help=(
            "This option allow you to show only target "
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help=("nodes to forcibly show." "ex), [-t 1-5], [-t 2-9 15 17], [-t 25-]."),
    )
    parser.add_argument(
        "-p",
        "--bring-node-pair-close-together",
        dest="close_pairs",
        required=False,
        nargs="*",
        default=[],
    ...
    parser.add_argument(
        ...
        default=[5.0, 3.0, 1.0],
        help=(
            "The weight of line. Number of elements in the list must"
