...
class TestAmber99:
    ...
    def get_bond_info(self):
        ...
        a_res = []
        for pair, kb, l_eq in zip(
            info["two_atoms"], info["force_consts"], info["length_eqs"]
        ):
            iatm, jatm = pair
        ...
        n = len(a_res)
        for (p, a_tbf), (iatm, jatm, tbf) in zip(
            a_res, self.gen_bonded_tbfs(bonded_tbfs, zero_tbfs)
        ):
