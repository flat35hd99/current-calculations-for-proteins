...
class FormatWrongError(Exception):
    pass
...
def graph_een(
    ...
    **kwds
    ...
    cc.set_line_thresholds(line_values, line_colors, line_thicks, line_weights)
...
class CommunicationChart:
    ...
    def add_data(self, don, acc, cond, *others):
        ...
        if not (is_forced1 or is_forced2):
            if not (nname1 in self.__targets or nname2 in self.__targets):
                ...
                if nname1 not in self.__don_targets:
                    return
                if nname2 not in self.__acc_targets:
                    return
        ...
        elif self.__opts["use_decrease"] and cond <= -self.__opts["tpl_threshold"]:
            self.__graph.add_node(nname1, label=label1, pin=True)
            self.__graph.add_node(nname2, label=label2, pin=True)
            self._rid_to_nname[rid1] = nname1
            self._rid_to_nname[rid2] = nname2
    ...
    def add_edge(self, nname1, nname2, cond, *others):
        ...
        if cond_old:
            if cond > cond_old:
                ...
                self.__graph.add_edge(nname1, nname2, weight=weight, style="invis")

        elif cond_old_rev:
            if cond > cond_old_rev:
                self._edge_to_cond[key] = cond
                color, width, weight, edge_label = self._get_edge_attrs(cond)
                self.__graph.add_edge(nname2, nname1, weight=weight, style="invis")

        else:
            self._edge_to_cond[key] = cond
            color, width, weight, edge_label = self._get_edge_attrs(cond)
            self.__graph.add_edge(key[0], key[1], weight=weight, style="invis")
    ...
    def show_edge(self, don, acc, cond, *others):
        ...
        if -thres < cond < thres:
            return
        if cond < 0.0 and not self.__opts["use_decrease"]:
            return
    ...
    def create_cluster(self, label, rids, **attributes):
        ...
        united_nnames = [
            self._rid_to_nname[rid] for rid in rids if self._rid_to_nname.get(rid)
        ]
        ...
        g = self.__graph.subgraph(
            nbunch=united_nnames,  # rank='same',
            name=name,
            label=label,
            **default_attributes
        )
    ...
    def create_legend(self):
      ...
      <tr><td port="i4">&nbsp;</td></tr>
        ...
        self.__graph.add_node(
            "key1", label=label_key1, color="white", fillcolor="white", fontsize=32
        )
        ...
        self.__graph.add_edge("key1:2", "key2:2", penwidth=10, **default_attributes)
        ...
        nodes = ["key1", "key2"]
        g = self.__graph.subgraph(
            nbunch=nodes, rank="sink", name=name, label="Legend", rankdir="LR"
        )  # **default_attributes)
