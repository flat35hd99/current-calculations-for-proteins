...
def pickup_respairs(
    ...
    prmtop_fmt,
    interval=1,
    cutoff_method="nearest",
    cutoff=5.0,
    trim_resnames=[],
    format="{rid:05}_{rname}",
    is_union=True,
    ext_resids="",
    is_both=False,
    **kwds
    ...
    rname_iatoms_pairs = list(
        (rname, list(numpy.array(iatoms)))
        for rname, iatoms in gen_residue_group(res_info, atom_info, name_fmt=format)
    )
    ...
    crd_parser = gen_trj(
        ...
        input_trj_fmt,
        trj_type="crd",
        input_fst_lst_int=[(1, -1, interval)],
        use_pbc=False,
    )
    crds = (crd for itraj, crd, box in crd_parser)

    res_beg_end_pairs = [
        (min(iatoms), max(iatoms)) for rname, iatoms in rname_iatoms_pairs
    ]
    ...
    respair_table_traj = (gen_respair_table_fortran(crd, resnames) for crd in crds)

    respair_iter = gen_respair_table_over_traj(respair_table_traj, is_union)
    ...
    def gen_excluded_respair(respairs, excluded_resids):
        ...
        for rname_i, rnames_j in respairs:
            ...
            for rname_j in rnames_j:
                ...
                if flag_i and rid_j in ext_resids:
                    continue
    ...
    if ext_resids and not is_both:
        ...
        for rname_i, rnames_j in gen_excluded_respair(respair_iter, ext_resids):
        ...
        for rname_i, rnames_j in gen_excluded_respair_both(respair_iter, ext_resids):
...
def gen_respair_table_fortran(crd, res_name_list):
    ...
    for ires_1 in range(n_res - 1):
        ...
        for jres_1 in range(ires_1 + 1, n_res):
            if cands[ires_1, jres_1]:
                j_res_list.append(res_name_list[jres_1])
