...
def gen_target_pairs(donor_line, acceptor_line, don_acc_pairs):
    ...
    for don, acc in don_acc_pairs:
        if don not in target_dons and acc not in target_accs:
            continue
        yield don, acc
...
def divide_flux(
    flux_fns, output_fn, dt=1.0, donor_line="", acceptor_line="", column_line="", **kwds
):
    """Divide flux file.
    ...
    # get target pairs
    target_pairs = list(gen_target_pairs(donor_line, acceptor_line, don_acc_pairs))
    target_indexes = []
    ...
    for don_name, acc_name in target_pairs:
        writer = FluxWriter(don_name, acc_name, output_fn, float(dt), labels)
