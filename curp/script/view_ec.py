...
def get_args():
    ...
    parser = argparse.ArgumentParser(description="Show 3D structure with EEN.")

    parser.add_argument(
        dest="ec_fps", nargs="+", metavar="EC_FP", help="specify the ec file paths."
    )
...
def parse_ec(ec_fp):
    ...
    lines = (line.strip() for line in file if line if not line.startswith("#"))
    ...
    for line in lines:
        ...
        yield dict(
            rid1=int(rid1), rname1=rname1, rid2=int(rid2), rname2=rname2, values=values
        )
...
def highlight_important_residues(filename, residue_shift=0):
    ...
    line_iter = (
        line.strip() for line in file if not line.isspace() if not line.startswith("#")
    )
