...
def main():
    ...
    rname_iatoms_pairs = list(
        (rname, list(numpy.array(iatoms) - 1))
    ...
    # get trajectory
    crd_parser = curp.gen_trj(args.traj_fns, natom, args.interval, use_pbc=False)
    ...
    cutoff_method = dict(
        ...
        farthest=lib_pickup.is_farthest,
    )[args.cutoff_method]
    ...
    method = gen_respair_table_par(resnames, rname_iatoms_pairs, cutoff_method, cutoff2)
    respair_table_traj = par.run(method, crd=crds)

    if par.is_root():
        respair_iter = gen_respair_table_over_traj(respair_table_traj, args.is_union)
    ...
    if single:
        ...
        respair_table_traj = (
            gen_respair_table(crd, resnames, rname_iatoms_pairs, cutoff_method, cutoff2)
...
def get_arguments():
    ...
    parser.add_argument(
        dest="traj_fns", nargs="+", help="specify the trajectory files."
    )
