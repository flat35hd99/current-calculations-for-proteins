...
def gen_fluxdata(flux_fn):
    ...
    try:
        ...
        print(
            "Warning: Unexpected error occured in loading flux data. Process continue."
        )
        no_axes = True
    ...
    for ipair_1, (donor, acceptor) in enumerate(zip(donors, acceptors)):
        print(
            "loading for {}/{} {} {} ... ** ".format(
                ipair_1 + 1, npair, donor, acceptor
            ),
            end="",
        )
        sys.stdout.flush()
...
class TransportCoefficientCalculator:
    ...
    def __init__(self, frame_range, avg_shift, nsample, coef, d_t=0.01):
    ...
    def run_mpi(self, data, **other):
        ...
        transport_coefficient = (
            self.__coef * self.d_t * pico_in_femto * numpy.trapz(acf, axis=0)[0]
        )

        print(
            "    cal time: {:.3f} [s] for {} {}".format(
                time.time() - t_0, donor, acceptor
            )
        )
...
class WriterBase:
    ...
    def setup(self):
        ...
        ncfile = netcdf.Dataset(
            self.__fp, clobber=True, mode="w", format="NETCDF3_64BIT"
        )
