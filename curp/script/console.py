...
def arg_conv_trj(parser=None):
    ...
    convert_only = sp.add_parser(
        "convert-only", help=("Convert the trajectory into" "other format.")
    )
    dryrun = sp.add_parser("dry-run", help="Do dry-run mode.")
    adjust_vel = sp.add_parser(
        "adjust-vel",
        help=("Adjust time of the velocity " "trajectory to t from t-dt/2."),
    )
    mask = sp.add_parser("mask", help="Remove solvent from trajectory.")
    dist = sp.add_parser("dist", help="Calculate inter-residue distances.")
    ...
    convert_only.set_defaults(func=top_n_trj(curp.script.do_convert_only))

    dryrun.set_defaults(func=top_n_trj(dryrun))

    adjust_vel.set_defaults(func=top_n_trj(curp.script.adjust_vel))

    mask.set_defaults(func=top_n_trj(curp.script.do_mask))
    mask.add_argument(
        "-m",
        "--mask-filename",
        metavar="MASK_FILENAME",
        dest="mask_fn",
        required=True,
        help=("The mask file name."),
    )

    dist.set_defaults(func=top_n_trj(curp.script.do_distance))
    dist.add_argument(
        "-m",
        "--method",
        metavar="DISTANCE_METHOD",
        dest="dist_method",
        required=True,
...
def arg_analyze(parser=None):
    ...
    # Definitions
    sp = parser.add_subparsers(dest="process", help=manual["analyze"])
    sp_divide = sp.add_parser("divide-flux", help=manual["divide-flux"])
    sp_respairs = sp.add_parser("pickup-respairs", help=manual["pickup-respairs"])
    sp_simplify = sp.add_parser("simplify-tensor", help=manual["simplify-tensor"])
    sp_sum_tc = sp.add_parser("sum-tc", help=manual["sum-tc"])
    sp_ncdata = sp.add_parser("get-ncdata", help=manual["get-ncdata"])
...
def arg_divide_flux(parser=None):
    ...
    parser.add_argument(
        "flux_fns",
        nargs="+",
        help=("Specify filenames in order. " "ex.) flux.dat0001, flux.dat0002, ..."),
    )
    parser.add_argument(
        "-o",
        "--output-filename",
...
def arg_respairs(parser=None):
    ...
    parser.add_argument(
        dest="trj_fns", nargs="+", action="append", help=("The trajectory file names.")
    )
    parser.add_argument(
        "-if",
        "--input-format",
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help=("Specify interval step to perform the calculation for " "trajectory."),
    )
    parser.add_argument(
        "-m",
        "--cutoff-method",
    ...
    parser.add_argument(
        ...
        action="store_false",
        help=(
            "Whether residue pair table is calculated by union "
    ...
    parser.add_argument(
        ...
        default="",
        nargs="*",
        help=("Residues that you want to exclude." "ex) 5:10 80:150"),
    )
    parser.add_argument(
        "-b",
        "--enable-residues-both-cxcluded",
        dest="is_both",
        required=False,
        action="store_true",
...
def arg_simplify(parser=None):
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Specify labels of components to analyze." 'ex.) -l "total,bond,angle,..."'
        ),
    )
    parser.add_argument(
        "fns",
...
def arg_sum_tc(parser=None):
    ...
    # add argument definitions
    parser.add_argument("tc_fps", nargs="+", help="Data files")
