...
def parse_options():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Calculate auto-correlation function data " "from energy flux datas."
        )
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "specify filenames of flux data. " "ex.) flux.dat0001, flux.dat0002, ..."
        ),
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default="",
        help=("specify a filename to output" "time series of transport coefficient."),
    )
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "The number of samples for one flux data file. "
...
def main():
    ...
    def gen_acf(flux_fns, tag=""):
        for ifn_1, flux_fn in enumerate(flux_fns):
            ...
            acf = cal_acf(
                ...
                frm_interval,
                opts.frm_shift,
                norm=opts.enable_norm,
                nsample=opts.nsample,
            )
            log.write("[{tag}] finished {number}".format(tag=tag, number=ifn_1 + 1))
            yield acf
    ...
    if opts.tcs_fn:  # and (not opts.enable_precise):
        ...
        if opts.use_debug:
            log.write("tc = {:22.14e} {:22.14e}".format(tc, tcs_avg[-1][0]))
        if opts.use_debug:
            log.write("disp = {:22.14e} {:22.14e}".format(tc_disp, tcs_disp[-1][0]))
