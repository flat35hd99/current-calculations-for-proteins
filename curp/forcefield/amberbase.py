...
class ConverterPrintable(object):
    def print_atom(self):
        ...
        atom = zip(info["names"], info["elems"], info["masses"], info["vdw_radii"])
        for iatm_1, (name, elem, mass, radius) in enumerate(atom):
            logger.info(
                "{:>7d}  {:<4s} {:<4s} {:12.7f} {:12.7f}".format(
    ...
    def print_vdw(self):
        logger.info("*** vdw ***")
        logger.info("{:>5} {:>5}{:>17}{:>17}".format("itype", "jtype", "c6", "c12"))
        ...
        for itype_1 in range(ntype):
            for jtype_1 in range(ntype):
                logger.info(
                    "{:>5} {:>5}  {:15.9E}  {:15.9E}".format(
                        itype_1 + 1,
                        jtype_1 + 1,
                        c6s[itype_1, jtype_1],
                        c12s[itype_1, jtype_1],
                    )
                )
    ...
    def print_ibnd_to_ipair(self):
        self._print_idx_to_ipair("bnd")

    def print_iang_to_ipair(self):
        self._print_idx_to_ipair("ang")

    def print_itor_to_ipair(self):
        self._print_idx_to_ipair("tor")

    def print_iimp_to_ipair(self):
        self._print_idx_to_ipair("imp")

    def print_i14_to_ipair(self):
        self._print_idx_to_ipair("14")
...
class TableMaker:
    ...
    def get_decomp_list(self, dtype="all"):
        ...
        if dtype == "bonded":
            ...
            return bonded14_list
        elif dtype == "bonded+":
            return bonded_list + bonded14_list
        elif dtype == "nonbonded":
            return nonbonded_list
        elif dtype == "nonbonded+":
            return nonbonded_list + bonded14_list
        else:
            return bonded_list + bonded14_list + nonbonded_list
    ...
    def get_ibnd_to_ipair(self):
        return self._get_idx_to_ipair("bnd")

    def get_iang_to_ipair(self):
        return self._get_idx_to_ipair("ang")

    def get_itor_to_ipair(self):
        return self._get_idx_to_ipair("tor")

    def get_iimp_to_ipair(self):
        return self._get_idx_to_ipair("imp")
    ...
    def _make_nonbonded_table(self):
        ...
        new_table = it.InteractionTable(base_table=nonbonded_table[:ntable].tolist())
        ...
        for minfo in self.get_mol_info():
            ...
            if tor_info:
                ...
                for imol in range(minfo["nmol"]):
                    ...
                    atoms_flags = zip(
                        tor_info["nonbonded_flags"], tor_info["four_atoms"]
                    )

                    two_atoms.extend(
                        [
                            (iatoms[0] + cur_natom, iatoms[3] + cur_natom)
                            for flag, iatoms in atoms_flags
                            if flag == 1
                        ]
                    )
    ...
    def _apply_target_to_bondtype(self, iatoms_list, info):
        ...
        for i_1, iatoms in enumerate(iatoms_list):
            if not self._contains(target_atoms, iatoms):
                continue
