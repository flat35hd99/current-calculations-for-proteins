...
class MaximumDepthError(CurpException):
    pass
...
if __name__ == "__main__":
    ...
    with Benchmarker(width=30) as bm:
        ...
        with bm("calculate average rdf"):
            rdfs = average_rdf(
                parser, rmax=rmax, dr=dr, interval=1, average=True, per_area=True
            )
        ...
        with bm("search well of rdf"):
            for iatm_1, tup in enumerate(gen_searched_wells(rdfs, dr, increment=5)):
                print(iatm_1 + 1)
                print_wells(tup)
