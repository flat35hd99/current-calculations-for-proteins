...
class NotFoundPropertyError(CurpException):
    pass


class NumberOfTargetError(CurpException):
    pass


class VolumeNotDefinedError(CurpException):
    pass


class TestError(CurpException):
    pass


class NumberOfGroupError(CurpException):
    pass
...
class UndefinedPropertyError(CurpException):
    pass
...
class VolumeCalculatorBase:
    ...
    def __init__(self, volume_setting):
        ...
        self.__gnames = [gname for gname, iatoms in self._setting.gname_iatoms_pairs]
    ...
    def get_gvolume(self, crd=[]):
        ...
        if self.__volumes is None:
            ...
            for igrp_1, (gname, atoms) in enumerate(self._setting.gname_iatoms_pairs):
    ...
    def write_group_volume(self, gvolumes):
        if not self.__do_write_gvolume:
            return
...
class VoronoiVolumeCalculatorBase(VolumeCalculatorBase):
    ...
    def get_gvolume(self, crd=[]):
        ...
        if self._setting.group_method == "united" and nohyd:
            ...
            for gname, iatoms in self._setting.gname_iatoms_pairs:
                ...
                if gvol is None:
                    msg = (
                        "At gname = {} volume cannot be defined. "
    ...
    def cal_voronoi(self, crd):
        ...
        try:
            ...
            # print(4)
            msg = (
                "When a solvated system is handled, "
        ...
        radii, volumes = lib_voro.cal_voronoi_all(
            crd, self._setting.target_atoms, enable_atoms
        )
        return radii, volumes
    ...
    def write_informations(self, mod):
        ...
        for ican_1 in range(mod.ncan):
            if mod.edges[ican_1] == 0:
                continue

            logger.debug(
                can_fmt.format(
                    mod.can_to_iatm[ican_1],
                    mod.edges[ican_1],
                    mod.can_crd[ican_1, 0],
                    mod.can_crd[ican_1, 1],
                    mod.can_crd[ican_1, 2],
                    sqrt(mod.can_len2[ican_1]),
                )
            )
        ...
        logger.debug(" {:>18s}   {:>36s}".format("INDICES", "RELATIVE POSITION"))
        ...
        for iver_1 in range(mod.nver):
            ...
            # ))
            logger.debug(
                " {:6}{:6}{:6}   {:12.5f}{:12.5f}{:12.5f}".format(
                    mod.can_to_iatm[ican - 1],
                    mod.can_to_iatm[jcan - 1],
                    mod.can_to_iatm[kcan - 1],
                    mod.ver_crd[iver_1, 0],
                    mod.ver_crd[iver_1, 1],
                    mod.ver_crd[iver_1, 2],
                )
            )
    ...
    def write_summary(self, nnabs, nabs_list):
        ...
        logger.debug(
            " {:5s}   {:5s}   {}".format("INDEX", "NABS", "... NEIGHBOURS INDICES")
        )
        ...
        for itar_1, (iatm, nnab, nab_list) in enumerate(
            ...
            logger.debug(" {:5}   {:5}   {}".format(iatm, nnab, nab_list[:nnab]))
            ...
            for jatm in nab_list[:nnab]:
                ...
                for katm in nab_j_list[:nnab_j]:
                    ...
                    logger.debug(" {:5} is not a neighbour of {:5}".format(iatm, jatm))
    ...
    def get_enable_atoms(self, crd=[]):
        if self.__is_enable_atoms is None:
            if self._setting.voronoi_no_hydrogen:
                ...
                is_enable_atoms = numpy.ones([self._setting.natom], numpy.bool)
...
class VoronoiVolumeCalculatorWithSolvation(VoronoiVolumeCalculatorBase):
    ...
    def solvate_crd(self, crd):
        ...
        if self._setting.voronoi_no_hydrogen:
            solv_crd, natom_solv = lib_sink.sink_nohyd(crd, self.__water_crd, probe)
        else:
    ...
    def write_crd(self, crd):
        ...
        self.__count += 1
        filename = "{}{:05}{}".format(self.__fn_base, self.__count, self.__fn_ext)
    ...
    def cal_volume(self, crd):
        try:
            ...
            if len(vols) != self.__ntar:
                ...
                raise NumberOfTargetError(msg.format(self.__ntar, len(vols)))
        ...
        if self.__gfile is None:
            ...
            try:
                ...
                if len(vols) != self.__ngrp:
                    ...
                    raise NumberOfTargetError(msg.format(self.__ngrp, len(vols)))
