...
def write_array(array, num_per_line=10):
    ...
    if length // num_per_line != 0:
        ...
        for iline_1 in range(length / num_per_line):
            ...
            line = " ".join("{:>5}".format(col) for col in array[icol_beg:icol_end])
...
def do_topology(setting):
    ...
    topology = parser.get_tplprm(file_format, fmt_section, potential, use_atype)
...
def do_target(atom_range, natom):
    ...
    target_atoms = target.parse_target_atoms_line(atom_range, natom)
...
def init_current(setting, par):
    ...
    target_anames = [
        str(i_atom) + "_" + atom_names[i_atom - 1] for i_atom in target_atoms
    ]
    ...
    gname_iatoms_pairs = group.get_group_iatoms_pairs(
        setting, target_atoms, topology.get_residue_info(), topology.get_atom_info()
    )
    ...
    if par.is_root():
        ...
        writer = get_writer(
            setting, decomp_list, target_anames, cal.get_groupnames(), gpair_table
        )
    else:
        writer = None
...
def get_data_iter(setting, topology, target_atoms):
    ...
    if use_classic:
        data_iter = parser.gen_phase_trajectory(
            file_format,
            fmt_section,
            setting.input.first_last_interval,
            natom,
            use_pbc,
            setting.curp.remove_trans,
            setting.curp.remove_rotate,
            target_atoms,
            masses,
            logger,
        )
        ...
        data_iter = parser.gen_matrix_ensemble(
            file_format, fmt_section, setting.input.first_last_interval, natom, logger
        )
...
def curp(
    ...
    **kwds
    ...
    if do_title:
        ...
        time_fmt = (
            "TIME STAMP: {year}/{month}/{day:02} " + "{hour:02}:{minute:02}:{second:02}"
        )
        time_stamp = time_fmt.format(
            year=now.year,
            month=now.month,
            day=now.day,
            hour=now.hour,
    ...
    if do_init and do_current:
        ...
        cal, writer, tpl, target_atoms, label_time_pairs_local = init_current(
            setting, par
        )
        ...
        # Initialize of curp to get the object for calculation.
        cal, writer, target_atoms, tpl, label_time_pairs_local = init_dynamics(
            setting, par
        )
        label_time_pairs += label_time_pairs_local
    ...
    if do_write:
        ...
        for istep, (cur_step, results) in enumerate(results_iter):
            ...
            logger.debug_cycle("    writing the data at step {} ...".format(cur_step))
            if par.is_root():
                t_write = time.time()
                writer.write(istep, *results)
                logger.debug_cycle(
                    "writing at {}: {}".format(cur_step, time.time() - t_write)
                )
            t_2 = time.time()
            dt_writing += t_2 - t_1
        ...
        if par.is_root():
            ...
            if istep > -1:
                logger.debug_cycle(
                    "total/1process : {} / {}".format(dt_cal, dt_cal / float(istep + 1))
                )
