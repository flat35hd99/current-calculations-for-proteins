...
def cal_rotation_omega(crd, vel, masses):
    ...
    I = numpy.array([[I_xx, I_xy, I_xz], [I_yx, I_yy, I_yz], [I_zx, I_zy, I_zz]])
...
def get_crd_vel_trans_rot_removed(
    ...
    if rem_trans:
        ...
        logger.info_cycle(msg.format("crd of center of mass", x_com, y_com, z_com, "A"))

        # get the center of mass for the velocity
        target_v_com = cal_com_crd_vel(target_vel, target_masses)
        vx_com, vy_com, vz_com = [
            float(v) for v in 1000.0 * target_v_com
        ]  # ndarray => float
        logger.info_cycle(
            msg.format("vel of center of mass", vx_com, vy_com, vz_com, "A/ps")
        )
    ...
    if rem_rotate:
        ...
        logger.info_cycle(
            msg.format("angular vel for rotation", x_omega, y_omega, z_omega, "rad/ps")
        )
...
if __name__ == "__main__":
    ...
    new_crd, new_vel = get_crd_vel_trans_rot_removed(mass, crd, vel, [1, 2], True, True)
