...
class FormatNotFoundError(CurpException):
    pass


class ParserNotFoundError(CurpException):
    pass


class ConverterNotFoundError(CurpException):
    pass


class NotFoundDictionary(CurpException):
    pass


class StepNumberInvalidError(CurpException):
    pass
...
def gen_phase_trajectory(
    ...
    if fmt_section.target == "restart":
        ...
        crd_pbcs = gen_trajectory(
            fmt_section.coordinate_file,
            CoordinateParser,
            natom,
            logger,
            use_pbc,
        ...
        vel_pbcs = gen_trajectory(
            fmt_section.velocity_file,
            VelocityParser,
            natom,
            logger,
            False,
    ...
    logger.info("first, last, interval of step = {} {} {}".format(first, last, inter))
    yield
    ...
    for (crd_data, vel_data) in it.izip(crd_pbcs, vel_pbcs):
        ...
        logger.info_cycle(
            "*** ISTEP = {}, CURRENT STEP = {} ***".format(istep, cstep_crd)
        )
        logger.debug_cycle()
