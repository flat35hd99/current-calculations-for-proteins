...
class Format2AmberBaseConverter(ConverterBase):
    ...
    def _convert_atom(self):
        ...
        self.__atom_info = dict(
            ids=ids, names=names, masses=masses, elems=elems, vdw_radii=vdw_radii
        )
    ...
    def __convert_torsion(self, if_fun):
        ...
        for itor in range(len(amb_atoms_hyd) / ncol):
            if not if_fun(amb_atoms_hyd[ncol * itor + 3]):
                continue
            iatoms = [
                abs(amb_atoms_hyd[ncol * itor + i]) / 3 + 1 for i in range(ncol - 1)
            ]
        ...
        for itor in range(len(amb_atoms_woh) / ncol):
            if not if_fun(amb_atoms_woh[ncol * itor + 3]):
                continue
            iatoms = [
                abs(amb_atoms_woh[ncol * itor + i]) / 3 + 1 for i in range(ncol - 1)
            ]
