...
class CalculatorBase(TimeStore):
    ...
    def prepare(
        self, topology, setting, target_atoms, gname_iatoms_pairs, interact_table
    ):
...
class CurrentCalculator(CalculatorBase):
    ...
    def prepare(self, *args, **kwds):
        ...
        self.__volume_obj = volume.get_volume_calculator(
            self.get_topology(),
            self.get_setting(),
    ...
    def run(self, data):
        ...
        logger.debug_cycle(
            "    calculating current values at step {} ...".format(cstep)
        )
...
class FluxCalculator(CalculatorBase):
    ...
    def run(self, data):
        ...
        cstep, (crd, vel, pbc) = data
        logger.debug_cycle("    calculating flux values at step {} ...".format(cstep))
