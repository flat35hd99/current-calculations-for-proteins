...
def old_gen_table_tbfs(pair_to_tbfs, table, natom):
    ...
    for tt in table:
        ...
        for iatm, jatm, tbf in pair_to_tbfs:
            if not (iatm_beg <= iatm <= iatm_end):
                continue
            if not (jatm_beg <= jatm <= jatm_end):
                continue
...
class TestStressTensorWithTarget:
    def test_kinetic_of_3_particle(self):
        ...
        # answer
        answer = list(gen_kinetic_current(vel, masses, target_vols, iatm_to_itars))
    ...
    def test_bonded_of_2_particles(self):
        ...
        crd = numpy.array([[1.0, 2.0, 3.0], [2.0, 1.0, 1.5]])
    ...
    def test_nonbonded_of_4_particles(self):
        crd = numpy.array(
            [[1.0, 2.0, 3.0], [2.0, 1.0, 1.5], [2.5, -1.0, -1.0], [-0.2, -0.3, -0.6]]
        )
        pair_to_tbfs = [
            (1, 2, numpy.array([0.2, 0.3, 0.4])),
            (1, 3, numpy.array([0.5, 0.6, 0.9])),
