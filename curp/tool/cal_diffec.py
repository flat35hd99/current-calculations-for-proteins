...
def main():
    ...
    ipair_to_pair1 = {
        (get_id(don), get_id(acc)): (don, acc) for (don, acc) in pair_to_tc1.keys()
    }

    ipair_to_pair2 = {
        (get_id(don), get_id(acc)): (don, acc) for (don, acc) in pair_to_tc2.keys()
    }

    ipair_to_tc1 = {
        (get_id(don), get_id(acc)): tc for (don, acc), tc in pair_to_tc1.items()
    }
    ipair_to_tc2 = {
        (get_id(don), get_id(acc)): tc for (don, acc), tc in pair_to_tc2.items()
    }
...
def parse_options():
    ...
    parser.add_argument(
        "tc_fps", nargs="+", help="specify filenames. "  # action='append',
    )
