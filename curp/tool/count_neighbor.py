...
if __name__ == "__main__":
    ...
    if sort_method == "seq":
        sorted_items = sorted(
            rkey_to_count.items(), key=lambda rkey: rkey[0][0], reverse=False
        )
    elif sort_method == "count":
        sorted_items = sorted(
            rkey_to_count.items(), key=lambda rkey: rkey[1], reverse=True
        )
    elif sort_method == "rev_count":
        sorted_items = sorted(
            rkey_to_count.items(), key=lambda rkey: rkey[1], reverse=False
        )
    else:
